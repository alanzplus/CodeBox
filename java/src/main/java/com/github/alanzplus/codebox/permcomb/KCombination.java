...
public enum KCombination {
    ...
    NO_REPETITION_ONE_TIME {
        @Override
        public List<List<Integer>> combination(final List<Integer> input, final int k) {
            final List<List<Integer>> combs = new ArrayList<>();
            helper(input, k, new ArrayList<>(), combs);
            return combs;
        }

        private void helper(final List<Integer> input, final int k, final List<Integer> comb, final List<List<Integer>> combs) {
            if (0 == k) {
    ...
    public abstract List<List<Integer>> combination(List<Integer> input, int k);
}
