...
public class ImmutableSparseArrayTest {
  ...
  public void testPageBased() throws Exception {
    {
      ImmutableSparseArray.PageBasedBuilder<Integer> builder =
          new ImmutableSparseArray.PageBasedBuilder<>(15);
      Arrays.asList(0, 1, 2, 3, 4, 5, 6).forEach(i -> builder.set(i, i));

      ImmutableSparseArray<Integer> array = builder.build();
      Assert.assertEquals(8,
          ((ImmutableSparseArray.PageBasedBuilder.PageBasedImpl) array).pageSize());
      Assert.assertEquals(SizeEstimator.intArrShallow(2) + SizeEstimator.refArrShallow(8),
          array.mem());
...

    {
      ...
      Assert.assertEquals(16,
          ((ImmutableSparseArray.PageBasedBuilder.PageBasedImpl) array).pageSize());
      Assert.assertEquals(SizeEstimator.intArrShallow(1) + SizeEstimator.refArrShallow(16),
          array.mem());
  ...
  public void testBinarySearchBased() throws Exception {
    {
      ImmutableSparseArray.BinarySearchBasedBuilder<Integer> builder =
          new ImmutableSparseArray.BinarySearchBasedBuilder<>();
      Arrays.asList(0, 1, 2, 3, 4, 5, 6).forEach(i -> builder.set(i, i));
      ImmutableSparseArray<Integer> array = builder.build();
      Assert.assertEquals(SizeEstimator.intArrShallow(7) + SizeEstimator.refArrShallow(7),
          array.mem());
      Arrays.asList(0, 1, 2, 3, 4, 5, 6).forEach(i -> Assert.assertEquals(i, array.get(i)));
      IntStream.range(7, 15).forEach(i -> Assert.assertNull(array.get(i)));
    }
